...
function confirmOverwrite(key: string): boolean {
...
function confirmReplace(): boolean {
  console.log('WARNING: --replace will DELETE remote KV keys not present in the backup.');
  console.log('Type PRUNE in full to confirm deletion of unmatched keys:');
  const rl = readline.createInterface({ input: process.stdin, output: process.stdout });
  return new Promise<boolean>((resolve) => {
    rl.question('> ', (answer) => {
      rl.close();
      resolve(answer.trim() === 'PRUNE');
    });
  });
}
...
export async function restore(backupFile?: string, opts: RestoreOptions = {}): Promise<RestoreResult> {
  ...
  if (validationErrors.length > 0) {
    ...
    throw new Error(`Backup validation failed for ${validationErrors.length} entr${validationErrors.length === 1 ? 'y' : 'ies'}; nothing was written`);
  }
  ...
  if (!opts.yes && !process.stdin.isTTY && !opts.dryRun) {
    throw new Error('Refusing to restore without confirmation: pass --yes after reviewing the backup');
  }
  if (!opts.yes && process.stdin.isTTY) {
    const rl = readline.createInterface({ input: process.stdin, output: process.stdout });
    const answer: string = await new Promise((resolve) => {
      rl.question(`Restore ${keys.length} entr${keys.length === 1 ? 'y' : 'ies'} to remote KV? (y/n): `, resolve);
    });
    rl.close();
  ...
  if (touchesReserved && !opts.dryRun) {
    ...
    if (!ok) {
      throw new Error(`Restore cancelled: confirmation for ${GLOBAL_AUTH_KV_KEY} overwrite not given`);
    }
  }

  // ---- Phase 3: optional prune requires strong confirmation BEFORE any writes ----
  if (opts.replace && !opts.confirmReplace) {
    throw new Error('Refusing to prune without strong confirmation (pass confirmReplace after reviewing keys)');
  }
...
if (process.argv[1] && process.argv[1].endsWith('restore-config.ts')) {
